...
# Тестовый вариант pve-autoinstall с урезанными ресурсами CT.
# Копия pve-autoinstall-test.sh, но с именем, начинающимся на test-,
# отдельным маркером, тегом логов и systemd-юнитом.

MARKER="/var/lib/test-pve-autoinstall/done"
LOG_TAG="test-pve-autoinstall"
...
ensure_nat_persistence() {
  ...
  local tmpfile
  tmpfile="$(mktemp /tmp/test-pve-autoinstall-interfaces.XXXXXX)"
...
# CT 103 Wireguard (test: минимальные ресурсы)
create_or_skip_ct 103 "Wireguard" 1 1 "local-lvm:4" "vmbrlab0" "192.168.10.10/24" "192.168.10.1" 1
apply_wireguard_networks 103

# CT 104 Stack (test: минимальные ресурсы)
create_or_skip_ct 104 "Stack" 2 2 "local-lvm:8" "vmbrlab0" "192.168.10.3/24" "192.168.10.1" 1
...
# Для тестовой версии оставляем ту же логику; при маленьком rootfs возможна ошибка
# инсталлятора (нехватка места), что нормально для теста.
...
# --- Finish ---
touch "$MARKER"
log "Autoinstall TEST completed successfully. Marker created: $MARKER"

# Отключаем тестовый сервис (ожидаем юнит test-pve-autoinstall.service)
systemctl disable --now test-pve-autoinstall.service >/dev/null 2>&1 || true
log "Test service disabled."
